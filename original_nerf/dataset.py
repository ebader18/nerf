...
def get_rays(datapath, mode):
    ...
    img_file_names = [f for f in os.listdir(datapath + f'/{mode}/imgs') if f.endswith('.png')]
    ...
    for i in range(N):
        ...
        distances[i] = np.sqrt(poses[i,0,3]**2 + poses[i,1,3]**2 + poses[i,2,3]**2)
        
        intrinsic = open(datapath + f'/{mode}/intrinsics/' + name).read().split()
        intrinsics[i] = np.array(intrinsic, dtype=float).reshape(4, 4)
        
        # Read images
        img = imageio.imread(datapath + f'/{mode}/imgs/' + name.replace('txt', 'png')) / 255.
